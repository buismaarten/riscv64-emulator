...
void test_inst32() {
    for (uint32_t i = 0; i < sizeof(program32) / sizeof(program32[0]); i++) {
        ...
        if (OP_AMOADD_D(inst)) {
            uint32_t aq = EXTRACT_BITS(inst, 26, 26);
            uint32_t rl = EXTRACT_BITS(inst, 25, 25);
            uint32_t rs2 = EXTRACT_BITS(inst, 24, 20);
            uint32_t rs1 = EXTRACT_BITS(inst, 19, 15);
            uint32_t rd = EXTRACT_BITS(inst, 11, 7);
        ...
        if (OP_BGE(inst)) {
            ...
            printf("Matched BGE instruction 0x%08X: rs2=%u, rs1=%u, offset=%d\n", inst, rs2, rs1, offset);
            count++;
        }
        ...
        if (OP_BGEU(inst)) {
            ...
            printf("Matched BGEU instruction 0x%08X: rs2=%u, rs1=%u, offset=%d\n", inst, rs2, rs1, offset);
            count++;
        }
        ...
        if (OP_BLT(inst)) {
            ...
            printf("Matched BLT instruction 0x%08X: rs2=%u, rs1=%u, offset=%d\n", inst, rs2, rs1, offset);
            count++;
        }
        ...
        if (OP_BLTU(inst)) {
            ...
            printf("Matched BLTU instruction 0x%08X: rs2=%u, rs1=%u, offset=%d\n", inst, rs2, rs1, offset);
            count++;
        }
        ...
        if (OP_BNE(inst)) {
            ...
            printf("Matched BNE instruction 0x%08X: rs2=%u, rs1=%u, offset=%d\n", inst, rs2, rs1, offset);
            count++;
        }

        if (OP_CSRRC(inst)) {
            uint32_t csr = EXTRACT_BITS(inst, 31, 20);
            uint32_t rs1 = EXTRACT_BITS(inst, 19, 15);
            uint32_t rd = EXTRACT_BITS(inst, 11, 7);

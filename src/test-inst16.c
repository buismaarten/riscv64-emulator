...
void test_inst16() {
    for (uint16_t i = 0; i < sizeof(program16) / sizeof(program16[0]); i++) {
        ...
        if (OP_C_ADD(inst)) {
            int16_t rd = EXTRACT_BITS3(inst, 11, 7);
            int16_t rs2 = EXTRACT_BITS3(inst, 6, 2);
        ...
        if (OP_C_ADDI4SPN(inst)) {
            ...
            if (nzuimm > 0) {
                int16_t rd = 8 + EXTRACT_BITS3(inst, 4, 2);
        ...
        if (OP_C_AND(inst)) {
            int16_t rd = 8 + EXTRACT_BITS3(inst, 9, 7);
            int16_t rs2 = 8 + EXTRACT_BITS3(inst, 4, 2);
        ...
        if (OP_C_JALR(inst)) {
            int16_t rs1 = EXTRACT_BITS3(inst, 11, 7);
